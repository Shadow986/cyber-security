...
class CyberSecToolkit {
    constructor() {
        Object.defineProperty(this, "outputPanel", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "modalBody", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    async portScan(target = '127.0.0.1') {
        ...
        for (const port of commonPorts) {
            ...
            const output = results.map(r => `Port ${r.port}: ${r.status.toUpperCase()} ${r.service ? `(${r.service})` : ''}`).join('\n');
    ...
    async vulnerabilityScan() {
        ...
        for (let i = 0; i < vulnerabilities.length; i++) {
            ...
            const output = vulnerabilities.slice(0, i + 1).map(v => `[${v.severity.toUpperCase()}] ${v.description} ${v.cve ? `(${v.cve})` : ''}`).join('\n');
            this.showOutput(`🛡️ Vulnerability Scan Results:\n\n${output}\n\n⚠️ Found ${i + 1} vulnerabilities`);
        }
    }
    async networkMapping() {
        ...
        for (let i = 0; i < devices.length; i++) {
            ...
            const output = devices.slice(0, i + 1).map(d => `${d.ip} - ${d.type} (${d.os})`).join('\n');
            this.showOutput(`🗺️ Network Discovery:\n\n${output}\n\nDevices found: ${i + 1}`);
        }
    }
    async hashAnalysis(hash = 'e99a18c428cb38d5f260853678922e03') {
...
window.startScan = () => toolkit.portScan();
window.showTools = () => document.getElementById('tools')?.scrollIntoView({ behavior: 'smooth' });
window.portScan = () => {
    const target = prompt('Enter target IP/hostname:', '127.0.0.1') || '127.0.0.1';
    toolkit.portScan(target);
};
window.vulnScan = () => toolkit.vulnerabilityScan();
window.networkMap = () => toolkit.networkMapping();
window.hashCracker = () => {
...
window.showPrevention = () => toolkit.showPreventionGuide();
window.closeModal = () => toolkit.closeModal();
